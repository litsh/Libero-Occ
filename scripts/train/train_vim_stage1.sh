...
EXP_NAME="${EXP_NAME:-vim_stage1_${PERSPECTIVE_VIEW_NAME}}"
...
for required in WORLD_MODEL_CKPT STAGE1_DATA_PATH ACTION_TOKENIZER_PATH; do
  if [[ -z "${!required}" ]]; then
...
torchrun \
  ...
  train/train_moe.py \
  --model_name_or_path "${WORLD_MODEL_CKPT}" \
  ...
  --visual_special_loss_weight "${VISUAL_SPECIAL_LOSS_WEIGHT:-1.0}" \
  --action_content_loss_weight "${ACTION_CONTENT_LOSS_WEIGHT:-0.0}" \
  --action_special_loss_weight "${ACTION_SPECIAL_LOSS_WEIGHT:-0.0}"
